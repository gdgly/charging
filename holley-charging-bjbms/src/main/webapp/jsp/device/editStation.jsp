			...
			<form class="form-horizontal" role="form" id="stationForm"
				...
				<div class="form-group">
					<label for="stationName" class="col-sm-2 control-label"><span
						style="color: red;">*</span>充电站名称：</label>
					<div class="col-sm-4">
				...
				<div class="form-group">
					<label for="editStationImg" class="col-sm-2 control-label hide"><span
					...
					<div class="col-sm-4 hide">
						<s:if test="#request.pobChargingStation.img != null">
							<img id="showEditStationImg" alt="ss" class="img-thumbnail"
								style="height: 20%; width: 50%;cursor: pointer;"
						...
						<input type="file" name="img" id="editStationImg" class="hide"/>
					</div>
					<label for="linkPhone" class="col-sm-2 control-label"><span
				...
				<label for="busMec" class="col-sm-2 control-label"><span
					...
					<div class="col-sm-4">
					<select id="busMec" name="busMec" class="form-control">
						<option value="0">系统平台</option>
						<s:iterator value="#request.companyList" status="statu" id="item">
						<s:if test="#request.pobChargingStation.busMec == #item.id">
				...
				<label for="stationToType" class="col-sm-2 control-label"><span
						style="color: red;">*</span>站类型：</label>
					<div class="col-sm-4">
						<select id="stationToType" name="stationToType"
							class="form-control" disabled="disabled">
							<option value="0">请选择</option>
							<s:iterator value="#request.stationToTypeList" status="statu" id="item">
							<s:if test="#request.pobChargingStation.stationToType == #item.value">
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-4">
						<button id="saveStationBtn" type="button" class="btn btn-warning btn-sm" disabled="disabled">保存</button>
						<button id="returnBtn" type="button" class="btn btn-warning btn-sm"  onclick="href('device/stationList.action');">返回</button>
					</div>
				</div>
			</form>
...
<script type="text/javascript">
	...
	$("#showEditStationImg").on("click",function(){
		$("#editStationImg").click();
	});
	var map;
	var point;
	var defaultSize = 15;
...
/////////////////////////////////
	...
	function initChangeSaveBtn(){
		...
		actionFun($("select[name='busMec']"),"change");
		actionFun($("select[name='stationToType']"),"change");
	}
///////////////////////////////////
	function cheack(station) {
		...
		}else if(station.stationToType == 0){
			return  "请选择站类型！！";
	...
	$(function() {
		...
		var msg;
		var param = {};
		var editStationByAjaxUrl = "device/station_editStationByAjax.action";
