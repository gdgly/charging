...
<div class="btn-group btn-group-sm floatSearch box-shadow">
		...
		<button title="设备告警" onclick="href('run/deviceAlarm.action');" type="button" class="btn btn-default"><span class="glyphicon glyphicon-warning-sign"></span></button>
</div>
	<div class="float box-shadow" id="detailPanel">
		<div style="height: 90px;padding-top: 26px;border-bottom: 1px solid #ccc;">
			<div class="col-sm-offset-1 col-sm-5">
				空闲中：<span style="color: #1ED538;">${freePile}</span>个
			</div>
			<div class="col-sm-6">
			...
			<div class="col-sm-6">
				故障：<span style="color: #FF8D18;">${badPile}</span>个
			</div>
			<%-- <div class="col-sm-offset-1  col-sm-5">
				忙碌中：<span style="color: #3EA2CC;">${chaPile}</span>个
			</div> --%>
		</div>
		...
		<s:iterator id="item" status="statu" value="#request.stationList">
			...
			<s:if test="stationToType ==1">
			<span><s:property value="stationToTypeDesc"/>站</span>
			</s:if>
			<s:else>
			<span style="color: green;"><s:property value="stationToTypeDesc"/>站</span>
			</s:else>
			</h5>
...
<script src="res/js/run/deviceMap.js"></script>
</body>
</html>
