...
function queryRealVerifyList(){
	...
	$.ajax({
		type:"POST",
		url:'business/busRealVerify_queryRealVerifyList.action',
		data:param,
		dataType:'json',
        cache: false,
             ...
             if(data.page){
            	 ...
            	 $(dataList).each(function(index,item){
            		 ...
            		 if(item.validStatus > 1){
            			 html += '<td><a class="a-blue" href="business/busRealVerifyDetail.action?busInfoId='+item.busInfoId+'&addTimeStr='+item.addTimeStr+'">详细</a></td>';
//            			 html += '<td><a class="a-disable" disabled="disabled" href="javascript:;">审核</a></td>';
            		 }else{
            			 html += '<td><a class="a-green" href="business/busRealVerify.action?busInfoId='+item.busInfoId+'&addTimeStr='+item.addTimeStr+'">审核</a></td>';
            		 }
