...
function initParams(){
	...
	$("#saveBtn").on("click",function(){
		...
		cardObj.startTime = $("#startTime").val();//启用日期
		cardObj.endTime = $("#endTime").val();//有效日期
		msg = checkFormData(userObj,cardObj);
		if(msg == "success"){
			if($("input[name='registerType']:checked").val()==2){
					addCard(cardObj);
			}else{
