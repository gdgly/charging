...
function checkFormData(userObj){
	...
	if(!REGBOX.regCn.test(userObj.realName)){
		...
		msg = "请选择市区！！";
	}else if(!isEmpty(userObj.email) && !REGBOX.regEmail.test(userObj.email)){
		msg = "请输入正确的邮箱！！";
	}else if(!isEmpty(userObj.cardNo) && (!REGBOX.regCard15.test(userObj.cardNo) && !REGBOX.regCard18.test(userObj.cardNo))){
		msg = "请输入正确的身份证号码！！";
