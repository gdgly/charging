...
function setTopMenuActive(){
	...
	var t = ":80/";
	if(basepath.indexOf(t) != -1){
		temp1 = basepath.substring(0,basepath.indexOf(t));
		temp2 = basepath.substring(basepath.indexOf(t)+t.length-1,basepath.length);
		temp3 = temp1+temp2;
		basepath = temp3;
	}
...
function verifyChangePwdForm(){
    ...
    if($("#newpwd").val()=="" || !passreg.test($("#newpwd").val())){
    	...
    	alert("密码必须为数字和字母组合并大于等于6位小于等于16位！！")
        return false;
    }else{
