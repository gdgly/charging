...
public class JxlXlsUtil {
    ...
    public void exportMoreXLS(String sheetName, String title, List<?> data, String[] properiesName, String[] headsName, Class<?> beanClass) throws Exception {
        AbstractXlsWriter xlsw = (AbstractXlsWriter) JxlXlsWriter.getInstance();
        String sname = sheetName;
        if (StringUtil.isEmpty(sname)) {
            sname = getRequest().getParameter("fileName");
        }
        xlsw.crateSheet(StringUtil.isEmpty(sname) ? "全部" : sname);
        xlsw.init();
        if (!StringUtil.isEmpty(title)) {
            xlsw.addTitle(title, headsName.length, false, Alignment.LEFT);
        }
    ...
    public void closeAndFlush() throws Exception {
        AbstractXlsWriter xlsw = (AbstractXlsWriter) JxlXlsWriter.getInstance();
        xlsw.end();
        xlsw.close();
        xlsw.getOut().flush();
     ...
     */
    public void exportMoreXLSInit(String sheetName, String title, List<?> data, String[] properiesName, String[] headsName, Class<?> beanClass) throws IOException, WriteException {
        String requestFileName = getRequest().getParameter("fileName");
        String fileName = DateUtil.DateToNosec10Str(new Date());
        if (StringUtil.isNotEmpty(requestFileName)) { // 可以后台修改文件名
        ...
        try {
            ...
            xlsw.start(StringUtil.isEmpty(sheetName) ? StringUtil.isEmpty(requestFileName) ? fileName : requestFileName : sheetName);
            if (!StringUtil.isEmpty(title)) {
                xlsw.addTitle(title, headsName.length, false, Alignment.LEFT);
            }
            xlsw.addHeader(headsName);
